...
- (void)setIsZX:(BOOL)isZX{
    _isZX = isZX;
    if (!isZX) {
        [xyLab removeFromSuperview];
    }
}
...
- (void)setPhoneLab:(NSString *)phoneLab{
    ...
    phone = [UILabel createLabelWithFrame:CGRectMake(65, 53, 90, 12) backgroundColor:CLEAN_COLOR textColor:Color_font_dark font:SYSTEM_FONT_(12) textalignment:NSTextAlignmentLeft text:phoneLab];
...
- (void)setTeleLab:(NSString *)teleLab{
    ...
    teleimg = [UIImageView createImageViewWithFrame:CGRectMake( 155, 53, 13, 13) backgroundColor:CLEAN_COLOR image:[UIImage imageNamed:@"线路_电话"]];
    [self addSubview:teleimg];
    
    tele = [UILabel createLabelWithFrame:CGRectMake(170, 53, 120, 12) backgroundColor:CLEAN_COLOR textColor:Color_font_dark font:SYSTEM_FONT_(12) textalignment:NSTextAlignmentLeft text:teleLab];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        xyLab = [UILabel createLabelWithFrame:CGRectMake(ScreenWidth-120, 80, 100, 20) backgroundColor:CLEAN_COLOR textColor:HEXCOLOR(0xed6900) font:SYSTEM_FONT_(16) textalignment:NSTextAlignmentCenter text:@"明佳信用专线"];
        xyLab.layer.borderWidth = 1;
        xyLab.layer.borderColor = HEXCOLOR(0xed6900).CGColor;
        xyLab.layer.cornerRadius = 4;
        [self addSubview:xyLab];
