...
#import "MJQuoteViewController.h"   // 报价查询
#import "MainTabBarController.h"
#import "MJInterOrderDefultViewController.h"  // 网上办单
//#import "MJDetailOrderViewController.h"    // 订单详情
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[model objectForKey:@"State"] isEqualToString:@"待受理"]) {
        [cell.typeBtn setImage:[UIImage imageNamed:@"修改b"] forState:UIControlStateNormal];
        ...
        [cell.typeBtn setImage:[UIImage imageNamed:@"修改b"] forState:UIControlStateNormal];
        if (typeSelected == 3) {
            [cell.typeBtn setImage:[UIImage imageNamed:@"已邀b"] forState:UIControlStateNormal];
    ...
    if ([[model objectForKey:@"Volume"] floatValue] > 0) {
        ...
        if ([[model objectForKey:@"Weight"] floatValue] > 0) {
            cell.productLab.text = [NSString stringWithFormat:@"%@ %@吨 %@方",[model objectForKey:@"CargoName"],[model objectForKey:@"Weight"],[model objectForKey:@"Volume"]];
        }
    }
    else if ([[model objectForKey:@"Weight"] floatValue] > 0) {
        ...
        cell.productLab.text = [NSString stringWithFormat:@"%@",[model objectForKey:@"CargoName"]];
    }
...
- (void)orderWithIndex:(int)indexRow andType:(orderType)type{
    ...
    switch (type) {
        case DAISHOULITYPE:
        {
            if ([[model objectForKey:@"OrderType"] isEqualToString:@"询价"]) {
        ...
        case DAIYUNSHUTYPE:
        {
            if (typeSelected == 1) {
                ...
                vc.myOrderorderId = [model objectForKey:@"OrderId"];
                [self.navigationController pushViewController:vc animated:NO];
            }
            if (typeSelected == 3) {
                // 评价
