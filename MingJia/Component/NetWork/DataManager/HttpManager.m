...
{
    Reachability *reachabilitys = [Reachability reachabilityWithHostname:@"www.baidu.com"];
    // NotReachable     - 没有网络连接
    // ReachableViaWWAN - 移动网络(2G、3G)
    // ReachableViaWiFi - WIFI网络
//    NSLog(@"reachablitys = %d",[reachabilitys currentReachabilityStatus]);
    return [reachabilitys currentReachabilityStatus];
...
//    debugLog(@"get request url: %@",[request.URL.absoluteString decode]);
...
//            debugLog(@"get responseObject:  %@",responseObject);
        }else{
